...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="NPY file contatning the surface samples",
)
parser.add_argument(
    "-s",
    "--sample",
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="NPY file contatning the octree grid corners for queries",
)
parser.add_argument("-o", "--output", type=str, required=True, help="output prefix")
parser.add_argument(
    "-wk", "--width_k", type=int, required=True, help="k in knn for width estimation"
)
parser.add_argument(
    "-wmin",
    "--width_min",
...
parser.add_argument(
    "-wmax", "--width_max", type=float, required=True, help="maximum width"
)
parser.add_argument(
    "-a", "--alpha", type=float, required=True, help="alpha for regularization"
)
...
if __name__ == "__main__":
    ...
    y_base_np = load_sample_from_npy(
        args.base, return_cupy=False, dtype=FLT_TYPE
    )  # [N_x, 3]
    if args.sample == args.base:
        x_sample_np = y_base_np
    else:
        x_sample_np = load_sample_from_npy(
            args.sample, return_cupy=False, dtype=FLT_TYPE
        )  # [N_y, 3]
