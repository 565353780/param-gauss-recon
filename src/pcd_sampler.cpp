...
PcdSampler::toFPSPcd(std::shared_ptr<open3d::geometry::PointCloud> &pcd,
  ...
  std::shared_ptr<open3d::geometry::PointCloud> down_sample_pcd =
      downSample(pcd, sample_point_num);

  return down_sample_pcd;
}

const bool PcdSampler::toFPSPcdFile(const std::string &pcd_file_path,
                                    const int &sample_point_num,
  ...
  const std::string save_pcd_folder_path =
      std::filesystem::path(save_pcd_file_path).parent_path();

  std::shared_ptr<open3d::geometry::PointCloud> down_sample_pcd =
      toFPSPcd(pcd, sample_point_num);
  ...
  if (!open3d::io::WritePointCloud(
          save_pcd_file_path, *down_sample_pcd,
