...
const bool PGRSolver::solve(const std::string &base, const std::string &query,
                            const std::string &out_prefix,
                            const PGRParams &pgr_params) {
  const torch::Tensor y_base =
      loadNpyData(base, pgr_params.dtype, pgr_params.device);
  if (y_base.size(0) == 0) {
    std::cout << "[ERROR][PGRSolver::solve]" << std::endl;
  ...
  std::cout << "[INFO][PGRSolver::solve]" << std::endl;
  std::cout << "\t x_width range : [" << x_width.min().item<float>() << " , "
            << x_width.max().item<float>()
            << "], mean : " << x_width.mean().item<float>() << std::endl;

  std::cout << "[INFO][PGRSolver::solve]" << std::endl;
  std::cout << "\t start solve the system..." << std::endl;
  const torch::Tensor lse =
      solveLSE(x_sample, y_base, x_width, CHUNK_SIZE, TARGET_ISO_VALUE,
               R_SQ_STOP_EPS, pgr_params);

  const torch::Tensor out_lse =
      torch::cat({y_base, -lse.reshape({3, -1}).permute({1, 0})}, 1);
  ...
  const torch::Tensor q_query =
      loadNpyData(query, x_sample.scalar_type(), x_sample.device());
  if (q_query.size(0) == 0) {
    std::cout << "[ERROR][PGRSolver::solve]" << std::endl;
    std::cout << "\t loadNpyData failed!" << std::endl;
  ...
  std::cout << "[INFO][PGRSolver::solve]" << std::endl;
  std::cout << "\t q_width range: [ " << q_width.min().item<float>() << " , "
            << q_width.max().item<float>() << " ]" << std::endl;

  const torch::Tensor sample_vals =
      get_query_vals(x_sample, x_width, y_base, lse, CHUNK_SIZE).cpu();
  const float iso_val = torch::median(sample_vals).item<float>();
  std::cout << "[INFO][PGRSolver::solve]" << std::endl;
  std::cout << "\t sample vals range: [ " << sample_vals.min().item<float>()
            << " , " << sample_vals.max().item<float>()
            << " ], mean: " << sample_vals.mean().item<float>()
            << " , median: " << torch::median(sample_vals).item<float>()
            << std::endl;
  ...
  const torch::Tensor query_vals =
      get_query_vals(q_query, q_width, y_base, lse, chunk_size);
  ...
  std::cout << "[INFO][PGRSolver::solve]" << std::endl;
  std::cout << "\t saving grid eval values to: " << out_eval_grid_npy
            << std::endl;
