...
const torch::Tensor get_A(const torch::Tensor &x, const torch::Tensor &y,
  ...
  const torch::Tensor inv_dist = torch::where(
      dist > x_width_unsqueezed, 1.0 / dist, 1.0 / x_width_unsqueezed);
  const torch::Tensor inv_cub_dist =
      inv_dist * inv_dist * inv_dist / 4.0 / M_PI;
...
const torch::Tensor mul_A_T(const torch::Tensor &x, const torch::Tensor &y,
  ...
  for (int i = 0; i < n_y_chunks; ++i) {
    ...
    const torch::Tensor lse_chunk_results =
...
const torch::Tensor get_B(const torch::Tensor &x, const torch::Tensor &y,
  ...
  const torch::TensorOptions opts =
      torch::TensorOptions().dtype(x.dtype()).device(x.device());
  const torch::TensorOptions bool_opts =
  ...
  for (int i = 0; i < n_row_chunks; ++i) {
    ...
    for (int j = i; j < n_col_chunks; ++j) {
      ...
      B.index_put_({row_chunk_slice, col_chunk_slice},
                   torch::einsum("ik,jk->ij", {A_block_i, A_block_j}));

      if (i != j) {
        B.index_put_(
            {col_chunk_slice, row_chunk_slice},
            B.index({row_chunk_slice, col_chunk_slice}).transpose(1, 0));
      } else {
        const int block_size =
            B.index({row_chunk_slice, col_chunk_slice}).size(0);
        if (block_size <= 0) {
          continue;
        }
